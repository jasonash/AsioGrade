...
export function LessonEditorPage({
  ...
  const handleExportPDF = async (): Promise<void> => {
    ...
    try {
      const result = await window.electronAPI.invoke<
        ServiceResult<{ pdfBase64: string; filename: string }>
      >('pdf:generateLessonPlan', currentLesson.id, course.name, unit.name)
  ...
  // Handle a complete lesson generated by AI (goals + structure + expansions in one)
  const handleAIFullLessonGenerated = async (
    goals: LearningGoal[],
    successCriteria: string[],
    ...
    // Replace goals and components with the generated ones
    await updateLesson({
      id: currentLesson.id,
      courseId: currentLesson.courseId,
      unitId: currentLesson.unitId,
          ...
          <Grid size={{ xs: 12, md: 4 }}>
            <Box sx={{ position: 'sticky', top: 16 }}>
              <LessonAIPanel
                ...
                onFullLessonGenerated={handleAIFullLessonGenerated}
              />
            </Box>
          </Grid>
        )}
